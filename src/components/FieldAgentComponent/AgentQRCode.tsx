...
// Compact, lightweight Model 2 QR Generator
class QR8bitByte {
  mode = 4;
  data: string;
...
const QRRSBlock = {
  ...
  getRsBlockTable(typeNumber: number, errorCorrectLevel: number) {
    switch (errorCorrectLevel) {
      ...
      default: return QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 3];
    }
  },
};
...
class QRCodeModel {
  ...
  private createBytes(buffer: QRBitBuffer, rsBlocks: Array<{ totalCount: number; dataCount: number }>) {
    let offset = 0;
    let maxDcCount = 0;
    let maxEcCount = 0;
...
export const AgentQRCode: React.FC<AgentQRCodeProps> = ({
  value,
  size = 68,
  color = "#0F172A",
  ...
  const { pathData, moduleCount } = useMemo(() => {
    try {
      const typeNumber = value.length > 32 ? 3 : 2;
      const qr = new QRCodeModel(typeNumber, 1);
      qr.addData(value || "AGT-0000");
      qr.make();
      ...
      return { pathData: "", moduleCount: 25 };
    }
  }, [value]);
  ...
  return (
    <View style={{ width: size, height: size, backgroundColor, borderRadius: 10, overflow: "hidden", alignItems: "center", justifyContent: "center" }}>
      <Svg width={size} height={size} viewBox={`0 0 ${viewBoxSize} ${viewBoxSize}`}>
        ...
        <Path d={pathData} transform={`translate(${padding}, ${padding})`} fill={color} />
      </Svg>
    </View>
  );
};
